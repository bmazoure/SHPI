...
class NeuralDice(object):
  ...
  def _get_average_value(self, network, env_step, policy):
    if self._solve_for_state_action_ratio:
      ...
      if self._categorical_action and self._num_samples is None:
        action_weights = tf.convert_to_tensor(policy.distribution(env_step.observation),np.float32)#.action.probs_parameter()
      ...
      flat_observations = tf.reshape(
          ...
          [batch_size * num_actions] + list(env_step.observation.shape[1:]) )
      ...
      return tf.reduce_sum(
          values * reverse_broadcast(action_weights, values), axis=1)
    else:
      return network((env_step.observation,))[0]
  ...
  def train_loss(self, initial_env_step, env_step, next_env_step, policy):
    ...
    if not self._solve_for_state_action_ratio:
      ...
      policy_log_probabilities = policy.distribution(
          env_step.observation,env_step.action)
      policy_ratio = tf.exp(policy_log_probabilities -
                            env_step.get_log_probability())
      policy_ratio = tf.cast(policy_ratio,tf.float32)

    bellman_residuals = (
        tf.cast(reverse_broadcast(discounts * policy_ratio, nu_values),tf.float32) *
  ...
  # @tf.function
  def train_step(self, initial_env_step: EnvStep,
                 env_step: EnvStep,
                 next_env_step: EnvStep,
  ...
  def estimate_average_reward(self, dataset: OffpolicyDataset,
                              target_policy: tf_policy.TFPolicy):
    ...
    def weight_fn(env_step):
      ...
      if not self._solve_for_state_action_ratio:
        ...
        policy_ratio = tf.exp(target_log_probabilities -
                              env_step.get_log_probability())
      return zeta * reverse_broadcast(policy_ratio, zeta)
    ...
    nu_zero = (1 - self._gamma) * get_fullbatch_average(
        dataset,
        limit=None,
        by_steps=False,
    ...
    dual_step = get_fullbatch_average(
        dataset,
        limit=None,
        by_steps=True,
  ...
  def _eval_constraint_and_regs(self, dataset: OffpolicyDataset,
                                target_policy: tf_policy.TFPolicy):
    ...
    discounts = self._gamma * next_env_step.discount
    bellman_residuals = (
        reverse_broadcast(discounts, nu_values) * next_nu_values -
