...
def _create_spec(env,_episode_step_limit):
    observation_spec = env.state_space
    action_spec = env.action_space
    observation_spec = specs.BoundedTensorSpec(shape=observation_spec.shape, dtype=tf.float32, name='obs_space', minimum=observation_spec.low, maximum=observation_spec.high)
    action_spec = specs.BoundedTensorSpec(shape=action_spec.shape, dtype=tf.int64, name='act_space', minimum=0, maximum=action_spec.n)
    ...
    step_num_spec = specs.tensor_spec.from_spec(
        specs.BoundedArraySpec([],
                               dtype=np.int64,
                               minimum=0,
                               maximum=_episode_step_limit,
                               name='step_num'))
    info_spec = PolicyInfo(log_probability=specs.BoundedTensorSpec(shape=(), dtype=tf.float32, name='log_probability', minimum=float('-inf'), maximum=0.))
    return EnvStep(tf_agents_time_step_spec.step_type, step_num_spec,
                   observation_spec, action_spec,
                   tf_agents_time_step_spec.reward,
                   tf_agents_time_step_spec.discount, info_spec,
                   {}, {})
